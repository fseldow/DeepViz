...
fig = plt.figure()
ax = fig.gca(projection='3d')
#ax.scatter(np.asarray(gd[:,0]), np.asarray(gd[:,1]),np.asarray(gd[:,2])+1, c='r', marker='o')

#ax.annotate(np.squeeze(trace[:,0]), np.squeeze(trace[:,1]),np.squeeze(trace[:,2]), arrowprops={'arrowstyle': '->', 'color': 'r', 'lw': 1},
#                    va='center', ha='center')
surf2 = ax.plot_trisurf(triangd, np.squeeze(dlow_dim), cmap=plt.cm.viridis,linewidth=0, antialiased=False)
